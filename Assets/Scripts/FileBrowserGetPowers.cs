...
public class FileBrowserGetPowers : MonoBehaviour
{
    ...
    IEnumerator ShowLoadDialogCoroutine()
    {
        ...
        if (FileBrowser.Success)
        {
            ...
            //if (!isGettingNiveau)
            //{
            //    WarningDiplayer.instance.DisplayText("le nom du fichier ne commence pas par un chiffre, veuillez selectionner à nouveau");
    ...
    private void ConvertFileToPowers(byte[] bytes)
    {
        ...
        for (int i = 1; i < strArray.Length; i++)
        {
            ...
            if (title != "")
            {
                ...
                Debug.Log("pouvoir ajouté   en " + power.niveau + " eme: " + power.title);
                powerList.Add(power);
